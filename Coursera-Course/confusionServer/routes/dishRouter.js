...
.post('/:dishId/comments/:commentId', authenticate.verifyUser, (req, res, next) => {
    res.statusCode = 403;
    res.end('Post operation not supported for the route /dishes/' + req.params.dishId +
        '/comments' + req.params.commentId );
})

.put('/:dishId/comments/:commentId', authenticate.verifyUser, (req, res, next) => {
    Dishes.findById(req.params.dishId)
    .then((dish) => {
        if( dish != null && dish.comments.id(req.params.commentId) != null ) {
            if( req.body.rating ) {
                dish.comments.id(req.params.commentId).rating = req.body.reating;
            }
            if( req.body.comment) {
...
.delete('/:dishId/comments/:commentId', authenticate.verifyUser, (req, res, next) => {
    Dishes.findById(req.params.dishId)
    .then((dish) => {
        if( dish != null && dish.comments.id(req.params.commentId) != null ) {
